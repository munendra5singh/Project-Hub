...
function startSubjectQuiz(subjectKey) {
    if (!quizData[subjectKey]) {
        alert('Quiz data for this subject is being prepared...');
        return;
    }
...
function renderQuestion(index) {
    ...
    if (diffBadge) {
        if (q.difficulty === 'easy') {
            ...
            diffBadge.innerHTML = `<i class="fa-solid fa-seedling"></i> 🟢 Easy Level (Q 1 - Q 25)`;
        } else if (q.difficulty === 'normal') {
            diffBadge.className = 'quiz-badge badge-normal';
            diffBadge.innerHTML = `<i class="fa-solid fa-fire text-amber-500"></i> 🟡 Normal Level (Q 26 - Q 50)`;
        } else if (q.difficulty === 'medium') {
            diffBadge.className = 'quiz-badge badge-medium';
            diffBadge.innerHTML = `<i class="fa-solid fa-bolt text-orange-500"></i> 🟠 Medium Level (Q 51 - Q 75)`;
        } else {
            diffBadge.className = 'quiz-badge badge-hard';
            diffBadge.innerHTML = `<i class="fa-solid fa-skull text-red-500"></i> 🔴 Hard Level (Q 76 - Q 100)`;
        }
    }

    // Question Text
    const qText = document.getElementById('quiz-question-text');
    if (qText) qText.textContent = `Question ${q.num}. ${q.q}`;
    ...
    if (userAns && userAns.submitted) {
        if (feedbackBox) {
            ...
            const userLetter = userAns.selected !== null ? String.fromCharCode(65 + userAns.selected) : 'Not Answered';
            const correctLetter = String.fromCharCode(65 + q.correct);
            
            let statusText = '';
            if (userAns.isSkipped) {
                statusText = `<span class="badge-skipped"><i class="fa-solid fa-circle-minus"></i> Skipped</span>`;
            } else if (userAns.isCorrect) {
                statusText = `<span class="badge-correct"><i class="fa-solid fa-circle-check"></i> Correct Answer!</span>`;
            } else {
                statusText = `<span class="badge-wrong"><i class="fa-solid fa-circle-xmark"></i> Incorrect Answer!</span>`;
            }
            ...
            feedbackBox.innerHTML = `
                <div class="feedback-header">
                    ...
                    <div class="text-sm font-semibold mt-1">
                        <span>Your Answer: <strong>${userLetter}</strong></span>
                        <span class="ml-4">Correct Answer: <strong class="text-emerald-500">${correctLetter}</strong></span>
                    </div>
                </div>
                <div class="feedback-why">
                    <strong><i class="fa-solid fa-lightbulb text-amber-400"></i> Why is this correct? Explanation:</strong>
                    <p class="mt-1">${q.why}</p>
                </div>
            `;
        }
        ...
        if (nextBtn) {
            ...
            if (index === 99) {
                nextBtn.innerHTML = `Complete Quiz & View Results <i class="fa-solid fa-trophy ml-2 text-amber-400"></i>`;
            } else if (index === 24 || index === 49 || index === 74) {
                nextBtn.innerHTML = `View Achievement & Proceed to Next Level <i class="fa-solid fa-arrow-right ml-2"></i>`;
            } else {
...
function handleNextQuestion() {
    ...
    if (curIdx === 24) {
        showMilestoneScreen(25, "🎉 Outstanding Performance!", "You have completed all 25 Easy questions!", "Welcome to Normal Level");
        return;
    } else if (curIdx === 49) {
        showMilestoneScreen(50, "🔥 Halfway Mark Reached!", "50 questions completed successfully!", "Welcome to Medium Level");
        return;
    } else if (curIdx === 74) {
        showMilestoneScreen(75, "⚡ Outstanding Progress!", "75 questions completed successfully!", "Welcome to Hard Level");
        return;
    } else if (curIdx === 99) {
        // Quiz Complete
...
function updateLiveProgressUI() {
    ...
    if (liveNum) liveNum.textContent = `Question ${curNum} / 100`;
