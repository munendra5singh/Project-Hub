    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            ...
            if (overlay && overlay.classList.contains('active')) {
                if (confirm('Do you want to exit the quiz? Your current progress will be lost.')) {
                    closeQuizOverlay();
                }
            }
        }
...
function startSubjectQuiz(subjectKey) {
    if (!quizData[subjectKey]) {
        alert('Quiz data for this subject is preparing...');
        return;
    }
...
function renderQuestion(index) {
    ...
    if (diffBadge) {
        if (q.difficulty === 'easy') {
            ...
            diffBadge.innerHTML = `<i class="fa-solid fa-seedling"></i> EASY LEVEL (Q1 - Q25)`;
            ...
            diffBadge.innerHTML = `<i class="fa-solid fa-skull text-red-500"></i> HARD LEVEL (Q76 - Q100)`;
        }
    }
    ...
    if (userAns && userAns.submitted) {
        if (feedbackBox) {
            ...
            if (userAns.isSkipped) {
                ...
                statusText = `<span class="badge-correct"><i class="fa-solid fa-circle-check"></i> Correct!</span>`;
            } else {
                statusText = `<span class="badge-wrong"><i class="fa-solid fa-circle-xmark"></i> Wrong!</span>`;
            }
        ...
        if (nextBtn) {
            ...
            if (index === 99) {
                nextBtn.innerHTML = `Complete Quiz & View Result <i class="fa-solid fa-trophy ml-2 text-amber-400"></i>`;
            } else if (index === 24 || index === 49 || index === 74) {
                nextBtn.innerHTML = `View Milestone & Next Level <i class="fa-solid fa-arrow-right ml-2"></i>`;
            } else {
...
function handleNextQuestion() {
    ...
    if (curIdx === 24) {
        showMilestoneScreen(25, "🎉 Great Progress!", "You have completed all 25 Easy Questions!", "Welcome to the Normal Level");
        return;
    } else if (curIdx === 49) {
        showMilestoneScreen(50, "🔥 Halfway There!", "50 Questions Completed Successfully!", "Welcome to the Medium Level");
        return;
    } else if (curIdx === 74) {
        showMilestoneScreen(75, "⚡ Outstanding!", "75 Questions Completed!", "Welcome to the Hard Level");
        return;
    } else if (curIdx === 99) {
        // Quiz Complete
