    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            ...
            if (overlay && overlay.classList.contains('active')) {
                if (confirm('क्या आप क्विज़ से बाहर निकलना चाहते हैं? आपकी प्रगति मिट जाएगी।')) {
                    closeQuizOverlay();
                }
            }
        }
...
function startSubjectQuiz(subjectKey) {
    if (!quizData[subjectKey]) {
        alert('इस विषय का क्विज़ डेटा तैयार हो रहा है...');
        return;
    }
...
function renderQuestion(index) {
    ...
    if (diffBadge) {
        if (q.difficulty === 'easy') {
            ...
            diffBadge.innerHTML = `<i class="fa-solid fa-seedling"></i> 🟢 आसान स्तर (प्र 1 - प्र 25)`;
        } else if (q.difficulty === 'normal') {
            diffBadge.className = 'quiz-badge badge-normal';
            diffBadge.innerHTML = `<i class="fa-solid fa-fire text-amber-500"></i> 🟡 सामान्य स्तर (प्र 26 - प्र 50)`;
        } else if (q.difficulty === 'medium') {
            diffBadge.className = 'quiz-badge badge-medium';
            diffBadge.innerHTML = `<i class="fa-solid fa-bolt text-orange-500"></i> 🟠 मध्यम स्तर (प्र 51 - प्र 75)`;
        } else {
            diffBadge.className = 'quiz-badge badge-hard';
            diffBadge.innerHTML = `<i class="fa-solid fa-skull text-red-500"></i> 🔴 कठिन स्तर (प्र 76 - प्र 100)`;
        }
    }

    // Question Text
    const qText = document.getElementById('quiz-question-text');
    if (qText) qText.textContent = `प्रश्न ${q.num}. ${q.q}`;
    ...
    if (userAns && userAns.submitted) {
        if (feedbackBox) {
            ...
            const userLetter = userAns.selected !== null ? String.fromCharCode(65 + userAns.selected) : 'उत्तर नहीं दिया';
            const correctLetter = String.fromCharCode(65 + q.correct);
            
            let statusText = '';
            if (userAns.isSkipped) {
                statusText = `<span class="badge-skipped"><i class="fa-solid fa-circle-minus"></i> छोड़ा गया</span>`;
            } else if (userAns.isCorrect) {
                statusText = `<span class="badge-correct"><i class="fa-solid fa-circle-check"></i> सही उत्तर!</span>`;
            } else {
                statusText = `<span class="badge-wrong"><i class="fa-solid fa-circle-xmark"></i> गलत उत्तर!</span>`;
            }
            ...
            feedbackBox.innerHTML = `
                <div class="feedback-header">
                    ...
                    <div class="text-sm font-semibold mt-1">
                        <span>आपका उत्तर: <strong>${userLetter}</strong></span>
                        <span class="ml-4">सही उत्तर: <strong class="text-emerald-500">${correctLetter}</strong></span>
                    </div>
                </div>
                <div class="feedback-why">
                    <strong><i class="fa-solid fa-lightbulb text-amber-400"></i> क्यों सही है? व्याख्या:</strong>
        ...
        if (nextBtn) {
            ...
            if (index === 99) {
                nextBtn.innerHTML = `क्विज़ पूरा करें और परिणाम देखें <i class="fa-solid fa-trophy ml-2 text-amber-400"></i>`;
            } else if (index === 24 || index === 49 || index === 74) {
                nextBtn.innerHTML = `उपलब्धि देखें और अगले स्तर पर जाएं <i class="fa-solid fa-arrow-right ml-2"></i>`;
            } else {
                nextBtn.innerHTML = `अगला प्रश्न <i class="fa-solid fa-arrow-right ml-2"></i>`;
            }
        }
    } else {
...
function handleNextQuestion() {
    ...
    if (curIdx === 24) {
        showMilestoneScreen(25, "🎉 शानदार प्रदर्शन!", "आपने सभी 25 आसान प्रश्न पूरे कर लिए हैं!", "सामान्य स्तर में आपका स्वागत है");
        return;
    } else if (curIdx === 49) {
        showMilestoneScreen(50, "🔥 आधा रास्ता पूरा!", "50 प्रश्न सफलतापूर्वक पूरे हुए!", "मध्यम स्तर में आपका स्वागत है");
        return;
    } else if (curIdx === 74) {
        showMilestoneScreen(75, "⚡ बेहतरीन प्रदर्शन!", "75 प्रश्न पूरे हो चुके हैं!", "कठिन स्तर में आपका स्वागत है");
        return;
    } else if (curIdx === 99) {
        // Quiz Complete
...
function showMilestoneScreen(count, heading, subtitle, nextMsg) {
    ...
    if (mCount) mCount.textContent = `${count} / 100 प्रश्न पूर्ण हुए`;
}
...
function updateLiveProgressUI() {
    ...
    if (liveNum) liveNum.textContent = `प्रश्न ${curNum} / 100`;
    if (liveComp) liveComp.textContent = `${completed} / 100 पूर्ण`;
    if (liveRem) liveRem.textContent = `${remaining} शेष`;
    ...
    if (elEasy) elEasy.textContent = `आसान: ${easyComp} / 25`;
    if (elNorm) elNorm.textContent = `सामान्य: ${normComp} / 25`;
    if (elMed) elMed.textContent = `मध्यम: ${medComp} / 25`;
    if (elHard) elHard.textContent = `कठिन: ${hardComp} / 25`;
}
